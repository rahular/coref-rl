...
class EmbeddingDictionary(object):
  ...
  def load_embedding_dict(self, path):
    ...
    if path:
      with open(path) as f:
        for i, line in enumerate(tqdm(list(f), desc="Loading word embeddings from {}".format(path), unit=" lines")):
          word_end = line.find(" ")
          word = line[:word_end]
          embedding = np.fromstring(line[word_end + 1:], np.float32, sep=" ")
          assert len(embedding) == self.size, "Bad embedding size: %d != %d at line %d of %s: %s" % (
            len(embedding), self.size, i, path, line)
