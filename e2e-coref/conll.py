...
IS_CONLL, BEGIN_DOCUMENT_REGEX = None, None
...
def evaluate_conll(gold_path, predictions, official_stdout=False, is_conll=False):
  global IS_CONLL, BEGIN_DOCUMENT_REGEX
  IS_CONLL = is_conll
  if IS_CONLL:
  ...
  result = { m: official_conll_eval(gold_file.name, prediction_file.name, m, official_stdout) for m in ("muc", "bcub", "ceafe") }
