...
class CorefModel(object):
    def __init__(self, config):
        self.config = config
        self.loss_func = self.config['loss']
        ...
        # self.queue = tf.PaddingFIFOQueue(capacity=10, dtypes=dtypes, shapes=shapes)
        # self.enqueue_op = self.queue.enqueue(self.queue_input_tensors)
        # self.input_tensors = self.queue.dequeue()
        self.input_tensors = [tf.placeholder(
            dtype, shape) for dtype, shape in input_props]
    ...
    def load_lm_embeddings(self, doc_key):
        ...
        #TODO: if you are using the 10k wikipedia documents for training, uncomment this line to create an External Link to the other hdf5 file!
        #group['ext link'] = h5py.ExternalLink("elmo_cache_wiki_10k_docs.hdf5", "/image/nlp-datasets/coref-rl/")
    ...
    def get_predictions_and_loss(self, tokens, context_word_emb, head_word_emb, lm_emb, char_index, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids):
        ...
        if self.loss_func != 'rl':
            ...
            self.pg_reward = tf.placeholder(shape=(None, None), dtype=tf.float32, name='pg_reward')  # [k, c + 1]
            loss = self.rl_loss(top_antecedent_scores, self.pg_reward)    # []
    ...
    def rl_loss(self, antecedent_scores, pg_reward):
        return tf.reduce_sum(-tf.matmul(tf.log(tf.nn.softmax(antecedent_scores) + 10**-6), pg_reward, transpose_b=True))
    ...
    def make_probs(self, lst):
        e_x = np.exp(lst - np.max(lst))
        return e_x / e_x.sum()

    def sample_predicted_antecedents(self, antecedents, antecedent_scores):
        predicted_antecedents, sampled_indices = [], []
        for idx, lst in enumerate(antecedent_scores):
            if idx > 0 and np.random.rand() < 0.1:
                sampled_indices.append(np.random.choice(list(range(len(lst[:idx]))), 1))
            else:
                probs = self.make_probs(lst)
                sampled_indices.append(np.random.choice(list(range(len(lst))), 1, p=probs))
        
        sampled_indices = np.array(sampled_indices).squeeze()
        for i, index in enumerate(sampled_indices - 1):
            if index < 0:
                predicted_antecedents.append(-1)
            else:
                predicted_antecedents.append(antecedents[i, index])
        return predicted_antecedents, sampled_indices
    ...
    def evaluate(self, session, official_stdout=False):
        ...
        conll_results = conll.evaluate_conll(
            self.config["conll_eval_path"], coref_predictions, official_stdout, self.config["is_conll"])
        average_f1 = sum(
